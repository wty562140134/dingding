package com.youtong.dingding.controller.IsvReceivController.IsvEnv.utils;
 ...
 * JDK6的下载地址：http://www.oracle.com/technetwork
 * /java/javase/downloads/jce-6-download-429243.html JDK7的下载地址：
 * http://www.oracle
 * .com/technetwork/java/javase/downloads/jce-7-download-432124.html
 */
public class DingTalkEncryptor {
	...
	public DingTalkEncryptor(String token, String encodingAesKey, String corpId)
		...
		if (null == encodingAesKey
			...
			throw new DingTalkEncryptException(
					DingTalkEncryptException.AES_KEY_ILLEGAL);
		}
		this.token = token;
		this.corpId = corpId;
	...
	public Map<String, String> getEncryptedMap(String plaintext,
		...
		if (null == nonce) {
			throw new DingTalkEncryptException(
					DingTalkEncryptException.ENCRYPTION_NONCE_ILLEGAL);
		}
		// 加密
		String encrypt = encrypt(Utils.getRandomStr(RANDOM_LENGTH), plaintext);
		String signature = getSignature(token, String.valueOf(timeStamp),
				nonce, encrypt);
	 ...
	 * @throws DingTalkEncryptException
	 */
	public String getDecryptMsg(String msgSignature, String timeStamp,
			String nonce, String encryptMsg) throws DingTalkEncryptException {
		// 校验签名
	...
	private String decrypt(String text) throws DingTalkEncryptException {
		...
		try {
			...
			throw new DingTalkEncryptException(
					DingTalkEncryptException.COMPUTE_DECRYPT_TEXT_ERROR);
		}
		...
		try {
			...
			plainText = new String(Arrays.copyOfRange(bytes, 20,
					20 + plainTextLegth), CHARSET);
			fromCorpid = new String(Arrays.copyOfRange(bytes,
					20 + plainTextLegth, bytes.length), CHARSET);
		} catch (Exception e) {
			throw new DingTalkEncryptException(
